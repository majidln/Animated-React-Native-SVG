...
export default function AnimatedStroke({d, progress}: Props): ReactElement {
    ...
    const bgStrokeAnimated = useAnimatedProps(() => ({strokeDashoffset: length - length * Easing.bezier(0.61, 1, 0.88,1 )(progress.value)}))
    const strokeAnimated = useAnimatedProps(() => ({strokeDashoffset: length - length * Easing.bezier(0.65, 0, 0.35,1 )(progress.value)}))
